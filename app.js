(function(){
  const VERSION = 'v2026.06.25-patch23-publish-checklist';
  ...
  function memberModeNotice(){const m=viewModeLabel(); return notice(`<b>${m.title}:</b> ${m.body}`);}
  ...
  function snapshotMetaBlock(){
    ...
    if(publicSnapshot.loaded && !connection.connected){
      return notice(`<b>Snapshot status:</b> read-only member snapshot published ${fmtDateTime(m.publishedAt)} · ${m.rowCount||liveLedger.transactions.length||0} transactions included.`);
    }
  ...
  function renderMoney(){
    ...
    const memberFriendlyCards = is2026
      ...
      : `${card('Fund Result',money(sm.total),'money collected minus money spent for '+scope, sm.total<0?'neg':'')}${card('Money Collected',money(sm.positive),'sales, credits, reimbursements, and top-offs')}${card('Money Spent',money(sm.negative),'tickets, parking, fees, travel, and other costs', sm.negative<0?'neg':'')}`;
    const plainIntro = is2026
      ...
      : '<b>Simple read:</b> this shows how much money came in, how much money went out, and what that means for the selected season. Use Settle to see who owes or is owed.';
    const salesCards = `${card('Ticket Sales',live?money(Math.max(0,sm.tickets)):money(m.ticketActivity),'ticket sales/resales in this view')}${card('Parking Sales',live?money(Math.max(0,sm.parking)):money(m.lifetimeParking),'parking money in this view')}${card('Ticket + Parking Costs',live?money(sm.negative):money(0),'money spent by the fund', sm.negative<0?'neg':'')}${card('Recent Money Moves','See below','latest ticket fund activity')}`;
    const dennisAudit = live ? `<details class="card"><summary><b>Dennis audit details</b></summary><p class="sub">Plain-English cards above are for members. This audit section keeps the raw ledger math available for Dennis.</p><div class="grid two">${card('Ledger Total',money(sm.total),'collected minus spent for '+scope, sm.total<0?'neg':'')}${card('Positive Rows',money(sm.positive),'raw positive TotalAmount rows')}${card('Negative Rows',money(sm.negative),'raw negative TotalAmount rows', sm.negative<0?'neg':'')}${card('Ticket Rows Net',money(sm.tickets),'all ticket rows in this view', sm.tickets<0?'neg':'')}${card('Parking Rows Net',money(sm.parking),'all parking rows in this view', sm.parking<0?'neg':'')}${card('Lifetime Regular Sales',money(m.lifetimeSales),'historical sales handled through the ledger')}</div><p class="eyebrow" style="margin-top:18px">Money Audit Rows</p>${auditTxnTable(scopeRows(),12)}</details>` : '';
  ...
  function renderParking(){
    ...
    layout('Parking','Parking Money Tracker','Parking is tracked by member. This page answers who has parking activity, what has been sold, and whether parking money affects the fund.',`${seasonSelectorBlock()}<p class="eyebrow" style="margin-top:26px">Parking Summary</p><div class="grid two">${headline}</div>${notice((live?scopeNote('Parking tracker')+' ':'')+'<b>Parking rule:</b> parking is member-level. It should affect the active members for the selected season, but it should not affect seat columns.')}<p class="eyebrow" style="margin-top:26px">Member Parking Balances</p><div class="grid">${pRows.map(p=>card(p.name,money(p.amount),p.amount===0?'settled / no open balance':`${p.count} parking ledger rows`,p.amount<0?'neg':'')).join('')}</div><p class="eyebrow" style="margin-top:26px">Parking Activity Rows</p>${live?memberActivityTable(parkingRows.sort((a,b)=>txSortValue(b).localeCompare(txSortValue(a))),20):refreshBlock()}<details class="card"><summary><b>Manager audit: parking totals</b></summary>${live?table(['Member','Parking Total','Rows Hit'],pRows.map(p=>[p.name,money(p.amount),p.count])):''}</details>`);
    bindSeasonSelector(); bindRefresh();
  }
